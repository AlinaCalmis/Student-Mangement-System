drop TYPE pers_stud_data;
--drop type pers_stud_data_rec;
----------- Get Student Personal DATA------------
create or replace type pers_stud_data_rec as object(
    f_name varchar2(50),
    l_name varchar2(50),
    study_year number(1),
    birth_date DATE,
...
create or replace type pers_stud_data as varray(1) of pers_stud_data_rec;
/

create or replace function getStudentPersonalData(st_id in number) return pers_stud_data
as 
...
create or replace function getProfessorData(p_id in number) return prof_data
as 
...
SET SERVEROUTPUT ON;
declare
    S pers_stud_data:= pers_stud_data();
--    P prof_data:= prof_data();
begin
    
    S.extend;
    S(1) :=  pers_stud_data_rec(null,null,null,null,null,null,null);
    dbms_output.put_line(S.count);
    S:= getStudentPersonalData(1001);
--    
--    p.extend;
